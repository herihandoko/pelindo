@php
$page_title = trans('admin.Admin | Edit contact item');
@endphp
@push('style')
    ...
    <div class="main-content">
        <section class="section">
            <div class="section-header">
                <h1>{{trans('admin.Edit Contact Item')}}</h1>
            </div>
            <a class="btn btn-primary mb-4" href="{{route('admin.footer-contact-item.index')}}" role="button"><i
                    class="fas fa-arrow-circle-left    "></i> {{trans('admin.Back')}}</a>
            <div class="section-body">
                <div class="card">
                    <div class="card-body">
                        <form action="{{route('admin.footer-contact-item.update', $item->id)}}" method="POST">
                            @csrf
                            @method('PUT')
