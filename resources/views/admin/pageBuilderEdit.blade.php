@php
$page_title = trans('admin.Admin | Edit page');
@endphp
@push('style')
                ...
                <div class="row">
                        ...
                        <div class="card elements-card">
                            <form action="{{route('admin.page-builder.update', $page->id)}}" method="post">
                                ...
                                <div class="card-body custom-padding">
                                    ...
                                    <input type="hidden" name="contents" value="{{$page->contents}}" required>
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="droppable p-4">
                            ...
                            <div data-id="{{$item}}" id="{{$item}}"
                                class="alert alert-primary alert-dismissible fade show w-100 ui-draggable ui-draggable-handle ui-draggable-dragging editable"
                                role="alert">
                                <strong>{{App\Models\PageComponent::find($item)->name}}</strong>
                                <button type="button" class="close" data-dismiss="alert"
                                    aria-label="Close" onclick="removeElement({{$item}})">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                                <img src="{{asset(App\Models\PageComponent::find($item)->image)}}" class="w-100">
                            </div>
                            @endforeach
                        </div>
...
@push('script')
    ...
    <script>
        let content = '{{$page->contents}}'.split(",");
